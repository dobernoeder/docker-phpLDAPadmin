# dobernoeder/phpldapadmin

[hub]: https://hub.docker.com/r/dobernoeder/phpldapadmin/

Latest release: 0.9.0 - phpLDAPadmin 1.2.5 - [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/dobernoeder/phpldapadmin/)
> Note: This is just a copy of osixia/phpldapadmin where curl is included for doing healt checks

**A docker image to run phpLDAPadmin.**
> [phpldapadmin.sourceforge.net](http://phpldapadmin.sourceforge.net)

- [dobernoeder/phpldapadmin](#osixiaphpldapadmin)
  ...
  - [Advanced User Guide](#advanced-user-guide)
    - [Extend dobernoeder/phpldapadmin:0.9.0 image](#extend-osixiaphpldapadmin090-image)
...
Run a phpLDAPadmin docker image by replacing `ldap.example.com` with your ldap host or IP :
    ...
    docker run -p 6443:443 \
            ...
            --detach dobernoeder/phpldapadmin:0.9.0
...
Example script:
    ...
    docker run --name phpldapadmin-service --hostname phpldapadmin-service --link ldap-service:ldap-host --env PHPLDAPADMIN_LDAP_HOSTS=ldap-host --detach dobernoeder/phpldapadmin:0.9.0
...
- Link your config file at run time to `/container/service/phpldapadmin/assets/config/config.php` :
            ...
            docker run --volume /data/my-config.php:/container/service/phpldapadmin/assets/config/config.php --detach dobernoeder/phpldapadmin:0.9.0
...
By default HTTPS is enable, a certificate is created with the container hostname (it can be set by docker run --hostname option eg: phpldapadmin.my-company.com).
	...
	docker run --hostname phpldapadmin.my-company.com --detach dobernoeder/phpldapadmin:0.9.0
...
Add --env PHPLDAPADMIN_HTTPS=false to the run command :
    ...
    docker run --env PHPLDAPADMIN_HTTPS=false --detach dobernoeder/phpldapadmin:0.9.0
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] dobernoeder/phpldapadmin:0.9.0 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach dobernoeder/phpldapadmin:0.9.0 --loglevel debug

See all command line options:

	docker run dobernoeder/phpldapadmin:0.9.0 --help
...
- **PHPLDAPADMIN_LDAP_HOSTS**: Set phpLDAPadmin server config. Defaults to :
  ...
  If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env PHPLDAPADMIN_LDAP_HOSTS="#PYTHON2BASH:[{'ldap.example.org': [{'server': [{'tls': True}]},{'login': [{'bind_id': 'cn=admin,dc=example,dc=org'}]}]}, 'ldap2.example.org', 'ldap3.example.org']" --detach dobernoeder/phpldapadmin:0.9.0
...
### Extend dobernoeder/phpldapadmin:0.9.0 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

    FROM dobernoeder/phpldapadmin:0.9.0
...
Clone this project :
	...
	git clone https://github.com/dobernoeder/docker-phpLDAPadmin
...
## Healt Check
The difference from the original image of osixia is that curl is included. You can do an easy health check with this image.
       docker run -p 6443:443 \
        --health-cmd "curl -fk https://127.0.0.1:443 || exit 1" \
        --health-interval 30s \
        --health-retries 3 \
        --health-timeout 15s \
        --detach dobernoeder/phpldapadmin:0.9.0

Change the ports when running a container without TLS/HTTPS, change also https:// to http://
       docker run -p 6080:80 \
        --env PHPLDAPADMIN_HTTPS=false \
        --health-cmd "curl -f http://127.0.0.1:80 || exit 1" \
        --health-interval 30s \
        --health-retries 3 \
        --health-timeout 15s \ 
        --detach dobernoeder/phpldapadmin:0.9.0
